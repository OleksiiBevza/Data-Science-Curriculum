...
def is_palindrome(word):
    ...
    new = word[::-1]
    if word == new:
        return 'The word is a polindrome'
    else:
        return 'The wors is not a polindrome'
...
def keep_consonants(word):
    ...
    """
    appear = []
    consonants = 'bcdfghjklmnpqrstvwxyz'  # String of consonants without commas
    word = word.lower()
...
def first_to_last_diff(s, ch):
    """ s is a string, ch is single character string
        Returns the difference between the index where c first
        occurs and the index where c last occurs. If ch does not 
        occur in s, returns -1. 
    """
    # your code here
    result = find(s, ch)
    if result != []:
        show = result[-1] - result[0]
    else:
        return '-1'
    return show
